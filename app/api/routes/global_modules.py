...
def global_workload(
    ...
    for u in users:
        ...
        # General Tasks vs Milestone Tasks split — same rule used everywhere
        # else in the app (assignments.py, AssignmentsPage, GlobalAssignments):
        # an assignment with no milestone_num and no custom_task_id is a
        # General Task; general_count + milestone_count always equals total.
        general_count   = sum(1 for a in assignments if not a.milestone_num and not a.custom_task_id)
        milestone_count = total - general_count
    ...
    summary = {
        ...
        "total_general":   sum(r["general_count"]   for r in result),
        "total_milestone": sum(r["milestone_count"] for r in result),
