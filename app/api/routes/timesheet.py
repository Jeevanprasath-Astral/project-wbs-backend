...
from app.core.permissions import is_timesheet_manager
...
def create_holiday(payload: HolidayCreate, db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
    if not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can manage holidays")
...
def create_leave(payload: LeaveCreate, db: Session = Depends(get_db),
                  current_user: User = Depends(get_current_user)):
    # Anyone can request leave for themselves; requesting on behalf of
    # someone else requires Admin/HR (timesheet management).
    if payload.user_id and payload.user_id != current_user.id and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can submit leave on behalf of someone else")
...
def update_leave(leave_id: int, payload: LeaveUpdate, db: Session = Depends(get_db),
    ...
    # Approving/rejecting (status) is Admin/HR only. The owner can still
    # edit their own pending request's type/reason before it's actioned.
    is_owner = l.user_id == current_user.id
    if payload.status is not None and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can approve or reject leave requests")
    if not is_owner and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can edit someone else's leave request")
...
def delete_leave(leave_id: int, db: Session = Depends(get_db),
    ...
    l = db.query(LeaveRequest).filter_by(id=leave_id).first()
    if not l: raise HTTPException(404, "Leave request not found")
    if l.user_id != current_user.id and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can delete someone else's leave request")
...
def create_permission(payload: PermissionCreate, db: Session = Depends(get_db),
                       current_user: User = Depends(get_current_user)):
    if payload.user_id and payload.user_id != current_user.id and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can submit permission/short-leave on behalf of someone else")
...
def update_permission(permission_id: int, payload: PermissionUpdate, db: Session = Depends(get_db),
    ...
    if payload.status is not None and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can approve or reject permission requests")
    if not is_owner and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can edit someone else's permission request")
    for k, v in payload.model_dump(exclude_none=True).items():
        setattr(p, k, v)
    db.commit()
    return {"status": "ok"}
...
def delete_permission(permission_id: int, db: Session = Depends(get_db),
    ...
    p = db.query(Permission).filter_by(id=permission_id).first()
    if not p: raise HTTPException(404, "Permission not found")
    if p.user_id != current_user.id and not is_timesheet_manager(current_user):
        raise HTTPException(403, "Only Admin or HR can delete someone else's permission request")
